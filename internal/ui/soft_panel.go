...
// Soft-panel chrome: the restyled overlay look used by every modal shell and by
// the Settings interior — rounded border with the title embedded in the top run,
// a left accent rail for focus instead of highlight blocks, quiet lowercase hint
// footers. Ported from tidemail, which forked this package.
//
// The feed manager's interior (renderManagerRow/renderManagerPicker and friends)
// still uses the older renderManager*/renderForm* chrome and can migrate here
// later; it currently gets the soft frame with legacy contents.
