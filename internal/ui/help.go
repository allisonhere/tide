	...
	sections := []section{
		...
		},
		{
			...
			entries: []entry{
				...
				{keys.VisualSelect.Help().Key, "character-wise selection in the content pane; h/l/j/k extend, v or esc cancels"},
				{keys.VisualLine.Help().Key, "line-wise selection; V or esc cancels, v switches to character-wise"},
		...
		},
		{
			...
			entries: []entry{
				...
				{"S → Feeds", "refresh every: background refresh interval in minutes; 0 = startup and f/F only"},
				{"S → Feeds", "delete read after: retention in days; 0 keeps everything, starred/summarized/unread never deleted"},
