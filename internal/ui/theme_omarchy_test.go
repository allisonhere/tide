...
// hostilePalettes are deliberately bad desktop palettes: colours that, taken
// literally, would produce an unreadable Tide UI. contrastCorrectTheme must
// pull every one up to Tide's contrast floors.
...
func TestResolveOmarchyThemeFallsBackWhenAbsent(t *testing.T) {
	...
	if merged.Bg != def.Bg || merged.Fg != def.Fg {
		t.Errorf("fallback theme = %+v, want default %q colours", merged, def.Name)
	}
}
