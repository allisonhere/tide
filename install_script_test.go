...
func TestInstallScriptDefaultsToUserLocalBinWithoutSudo(t *testing.T) {
	tmp := t.TempDir()
	archive := writeInstallTestArchive(t, tmp, "tide test-version", 0)
	...
	installed := filepath.Join(tmp, ".local", "bin", "tide")
...
func TestInstallScriptRemovesWritableStaleBinaryEarlierOnPath(t *testing.T) {
	...
	stale := filepath.Join(staleBin, "tide")
...
func TestInstallScriptLeavesExistingBinaryWhenDownloadedBinaryFailsVersionCheck(t *testing.T) {
	...
	installed := filepath.Join(installDir, "tide")
...
func writeInstallTestArchive(t *testing.T, dir, version string, exitCode int) string {
	t.Helper()
	path := filepath.Join(dir, "tide-linux-x86_64.tar.gz")
	var buf bytes.Buffer
	gz := gzip.NewWriter(&buf)
	tw := tar.NewWriter(gz)
	body := "#!/bin/sh\nif [ \"${1:-}\" = \"--version\" ]; then echo " + shellQuote(version) + "; exit " + strconv.Itoa(exitCode) + "; fi\n"
	if err := tw.WriteHeader(&tar.Header{Name: "tide-linux-x86_64", Mode: 0o755, Size: int64(len(body))}); err != nil {
		t.Fatal(err)
	}
...
func assertNoInstallTemps(t *testing.T, dir string) {
	t.Helper()
	matches, err := filepath.Glob(filepath.Join(dir, ".tide.tmp.*"))
	if err != nil {
		t.Fatal(err)
	}
	if len(matches) > 0 {
